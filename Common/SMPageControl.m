...
{
	_numberOfPages = 0;
	_tapBehavior = SMPageControlTapBehaviorJump;
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    id va = [self.pageRects objectAtIndex:0];
    CGRect rect = [va CGRectValue];
    if (CGRectContainsPoint(rect, point) || (rect.origin.x >point.x && rect.origin.x-point.x<=5))
    {
        self.tapBehavior =SMPageControlTapBehaviorJump;
        [self setCurrentPage:0 sendEvent:YES canDefer:YES];
        return;

    }
    else
    {
        self.tapBehavior =SMPageControlTapBehaviorStep;
    }
